...
def read_data():    
    df1 = pd.read_csv("comb.csv")
    df2 = pd.read_csv("entertainment.csv")
    df3 = pd.read_csv("tech.csv")
    print("missing values in comb.csv \n",df1.isnull().sum())
    print("missing values in entertainment.csv \n",df2.isnull().sum())
    print("missing values in tech.csv \n",df3.isnull().sum())
    df = pd.concat([df1,df2,df3],axis = 0,ignore_index =True)
    print("missing values in final data",df.isnull().sum())
    return df
